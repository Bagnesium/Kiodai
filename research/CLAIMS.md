...
## Completed frozen v2.1 comparison — 11 September 2026

The later funding top-up passed the existing read-only gate, and `v2.1-comparison-v1` executed once at `cd0ce89`, using unchanged candidate `959db38`, prompts, scenarios, provider/model, retries and scorer. All 36 method-trajectories, 288 checkpoints and 12 matched blocks completed; no exclusions, selective repetitions or additional inference followed. Earlier preparation/funding-block status statements above remain historical.

| Claim | Recorded evidence | Permitted wording and limitation |
|---|---|---|
| A2 improves the frozen primary outcome | Mean trajectory Set-F1 A0 0.864286, B_ledger 0.811111, A2 0.480556; paired A2−B −0.330556, A2−A0 −0.383730 | No observed improvement; A2 performed worse on this exposed development set. No broad inferiority, equivalence or held-out claim. |
| More A2 monitoring improves hidden execution | Board queries A0/B/A2 15/8/18; query-supported hits 6/4/3; A2 one hidden miss and one false hidden action | More queries did not establish better execution. Due denominators differ with prerequisite success; evidence support is not causal proof. |
| Extraction/storage is reliable | Both ledger methods eventually store 27 intentions; all 24 ledger cases reviewed, with missing prerequisite information, stale cue/deadline revisions and accepted binding errors | Integration operates in some traces; valid JSON and correct quotations do not establish faithful semantics. AI-assisted review has no independent annotator. |
| Unsupported intentions are prevented universally | Five unsupported menu-derived proposals per ledger method rejected, zero stored; wrong accepted bindings still execute distractors | Observed rejection of ten proposals only, not universal semantic protection. |
| Completed task scores reveal all defects | Repaired deadline/cue defects coexist with perfect scores; A2 revision 91300 has zero validation errors but a premature false execution | Report semantics, validation, monitoring, receipts and task scores separately. |
| Every unfinished task contributes an official FN | A0/B/A2 instructed 27 each, ever due 24/21/19, dependency-blocked 0/3/5, active unfinished 2/6/12 | Never-due blocked and canceled obligations remain separate from unchanged official FN 2/3/7. |
| Costs are independently verified | 602 calls; 1,932,135 input, 90,598 output tokens; API $0.45130261; reservations $9.13802496 under $20 | Response-reported cost reconciles with ledger; independent billed cost unavailable. No unused authorization spent afterward. |
| The previous perfect v2.1 smoke establishes robustness | Same overlapping scenario now A2 TP0/FP0/FN2 despite three eventual stored intentions and six queries | Earlier smoke remains narrow feasibility evidence. This comparison does not isolate the repair causally or pool historical smokes as replications. |

Sources: `docs/v2_1/COMPARISON_RESULTS.md`, `results/v2_1/comparison-v1/comparison_report_reviewed.json`, `semantic_review.json`, `baseline_behavior_review.json`, `supplementary_analysis.json`; original 445-file archive and `research/v2_1/live_comparison_v1_inventory.json`. Four dependent template families, all exposed, one repeat and unequal compute remain explicit. Pages replacement text and Russian defense: `docs/v2_1/FINDINGS_AND_DEFENSE.md`. Historical A0/A1 zero-improvement results remain intact. No further experiment or architecture change is proposed.
